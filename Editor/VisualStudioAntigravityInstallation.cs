...
namespace Microsoft.Unity.VisualStudio.Editor
{
	internal class VisualStudioAntigravityInstallation : VisualStudioInstallation
	{
		...
		public static bool TryDiscoverInstallation(string editorPath, out IVisualStudioInstallation installation)
		{
			...
			TryGetApplicationVersion(editorPath, out version);
			...
			try
			{
				...
				// but we can link to [antigravity]/antigravity or [antigravity]/bin/antigravity
				...
				if (File.Exists(manifestFullPath))
				{
					...
					Version.TryParse(manifest.version.Split('-').First(), out manifestVersion);
					if (version == null)
		...
		private static bool TryParseVersionString(string input, out Version version)
		{
			...
			var trimmed = input.Split(new[] { ' ', '\t', '(' }, StringSplitOptions.RemoveEmptyEntries).FirstOrDefault();
			if (string.IsNullOrEmpty(trimmed))
				return false;

			trimmed = trimmed.Split('-').FirstOrDefault() ?? trimmed;
			return Version.TryParse(trimmed, out version);
		}

		private static bool TryGetApplicationVersion(string editorPath, out Version version)
		{
			version = null;
			try
			{
#if UNITY_EDITOR_WIN
				var info = FileVersionInfo.GetVersionInfo(GetRealPath(editorPath));
				if (TryParseVersionString(info.ProductVersion, out version))
					return true;
				if (TryParseVersionString(info.FileVersion, out version))
					return true;
				return false;
#elif UNITY_EDITOR_OSX
				var plistPath = IOPath.Combine(GetRealPath(editorPath), "Contents", "Info.plist");
				if (!File.Exists(plistPath))
					return false;

				var content = File.ReadAllText(plistPath);
				var shortVersion = Regex.Match(content, "<key>CFBundleShortVersionString</key>\\s*<string>([^<]+)</string>", RegexOptions.IgnoreCase);
				if (shortVersion.Success && TryParseVersionString(shortVersion.Groups[1].Value, out version))
					return true;

				var bundleVersion = Regex.Match(content, "<key>CFBundleVersion</key>\\s*<string>([^<]+)</string>", RegexOptions.IgnoreCase);
				if (bundleVersion.Success && TryParseVersionString(bundleVersion.Groups[1].Value, out version))
					return true;
